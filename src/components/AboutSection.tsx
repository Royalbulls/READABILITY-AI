...
export default function AboutSection({ onBackToWorkspace }: AboutSectionProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-6 animate-fadeIn py-1.5">
      {/* Introduction Banner */}
      <section className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-sm flex flex-col md:flex-row items-center gap-6">
        <div className="md:w-2/3 flex flex-col gap-3">
          ...
          <h2 className="font-display font-extrabold text-xl md:text-2xl text-slate-950 tracking-tight leading-tight">
        ...
        <div className="md:w-1/3 w-full flex justify-center">
          <div className="relative w-32 h-32 bg-slate-50 border border-slate-200 rounded-3xl flex flex-col items-center justify-center p-3 text-center group shadow-inner">
            <div className="absolute inset-0 bg-radial-gradient from-blue-50/20 via-transparent to-transparent opacity-50 rounded-3xl" />
            <Shield className="w-10 h-10 text-slate-900 mb-1.5 transition-transform group-hover:scale-110 duration-500" />
      ...
      <section className="bg-slate-900 text-white rounded-3xl p-5 md:p-6 border border-slate-800 shadow-xl flex flex-col md:flex-row gap-6 items-center relative overflow-hidden">
        ...
        <div className="md:w-1/3 flex flex-col items-center text-center z-10">
          <div className="w-16 h-16 bg-slate-800 border border-slate-700 rounded-2xl flex items-center justify-center text-indigo-400 mb-2 shadow-lg">
            <Cpu className="w-8 h-8 animate-pulse" />
          </div>
          <h3 className="font-display font-extrabold text-xs uppercase tracking-wider text-white">
            MR. KILVISH
          </h3>
          <p className="text-[9px] font-mono text-slate-400 uppercase font-bold tracking-widest mt-0.5">
            CORE COGNITIVE DEFENDER
          </p>
        </div>

        <div className="md:w-2/3 flex flex-col gap-3 z-10">
          <h4 className="font-display font-bold text-sm text-blue-400">
            Who is Mr. Kilvish?
          </h4>
          ...
          <div className="text-[11px] font-mono text-indigo-300 italic">
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {manifestoPoints.map((point, idx) => (
            <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col gap-2">
              <span className="text-[9px] font-mono text-blue-600 font-extrabold">0{idx + 1} / CONCEPT</span>
      ...
      <section className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          ...
          <h3 className="font-display font-bold text-lg text-slate-900 tracking-tight flex items-center gap-2">
            <HelpCircle className="w-4.5 h-4.5 text-slate-950 shrink-0" />
            Frequently Asked Questions
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {faqItems.map((item, idx) => (
            <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col gap-1.5">
      ...
      {/* CTA Box */}
      <section className="bg-slate-50 border border-slate-200 rounded-2xl p-5 text-center flex flex-col items-center gap-2.5">
        ...
        <button
          ...
          className="mt-1 px-5 py-2 bg-slate-950 hover:bg-slate-850 text-white font-display font-bold text-[11px] uppercase tracking-wider rounded-xl transition-all shadow-md flex items-center gap-2 cursor-pointer"
        >
